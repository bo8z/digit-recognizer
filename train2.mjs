...
class Network {
    ...
    forward(input) {
        ...
        // Iterate over each non-input neuron layer.
        for (let neuronLayer = 0; neuronLayer < this.biases.length; neuronLayer++) {
            const nextActivations = [];
            const isOutputLayer = neuronLayer === this.biases.length - 1;

            // Then iterate over each neuron in that layer.
            for (let neuron = 0; neuron < this.biases[neuronLayer].length; neuron++) {
                let weightedSum = this.biases[neuronLayer][neuron];

                for (let sourceNeuron = 0; sourceNeuron < activations.length; sourceNeuron++) {
                    weightedSum = weightedSum.add(
                        this.weights[neuronLayer][neuron][sourceNeuron].mul(activations[sourceNeuron])
                    );
                }
